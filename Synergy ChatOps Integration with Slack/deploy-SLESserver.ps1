...
function deploy-slesserver {
    ...
    $serverprofiletemplate = "SLES 12 deployment with Streamer"
...
"@
    ...
    try {
        ...
        $ip = (get-hpovserverprofile -Name $name).osDeploymentSettings.osCustomAttributes | ? name -eq Bond0NIC1.ipaddress | % value
